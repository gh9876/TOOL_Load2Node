...
def select_synthload_profile(folder_path, year_of_interest, use_excel, excel_profiles_path=None):
    ...
    if year_of_interest < 2018:
        print(f"Warning: Year of interest {year_of_interest} is earlier than the available data (2018). Using 2018 instead.")
        year_of_interest = 2018
    elif year_of_interest > 2029:
        print(f"Warning: Year of interest {year_of_interest} is later than the latest available data (2029). Using 2029 instead.")
    ...
    if use_excel:
        if excel_profiles_path and os.path.exists(excel_profiles_path):
            print(f"Using Excel file: {excel_profiles_path}")
    ...
    for file_name in os.listdir(folder_path):
        if file_name.endswith('.csv') and year_of_interest in file_name:
            ...
            print(f"Selected CSV profile for year {year_of_interest}: {file_name}")
            return file_path
...
def create_hourly_profiles(file_path_profiles, year_of_interest, use_excel):
    ...
    def read_and_organize_csv(file_path):
        ...
        with open(file_path, mode='r', encoding='utf-8-sig') as file:
            ...
            for row in csv_reader:
                try:
                    ...
                    print(f"IndexError: {e}. Row data: {row}")
                    continue
            print("Created synthetic load profiles\n")
        ...
        print(f"Number of rows after resampling: {len(hourly_profiles)}")
    ...
    def process_and_resample_csv(file_path_profiles, year_of_interest):
        ...
        """
        print("Reading and organizing CSV data...")
        ...
        print("Converted 'Zeit' column to datetime.")
        df.set_index(['Typnummer', 'Zeit'], inplace=True)
        print("Set 'Typnummer' and 'Zeit' as index.")

        hourly_profiles = df.groupby('Typnummer').resample('h', level='Zeit').mean().reset_index()
        print(f"Resampled data to hourly values. Number of rows after resampling: {len(hourly_profiles)}")
        ...
        for typnummer, df_group in hourly_profiles.groupby('Typnummer'):
            if df_group['Wert'].nunique() == 1:
                ...
                print(f"Warning: Detected constant profile for Typnummer {typnummer} with value {constant_profiles[typnummer]}")

        # Remove February 29th for leap years
        is_leap_year = calendar.isleap(int(year_of_interest))
        if is_leap_year:
            print(f"Leap year detected for {year_of_interest}. Removing February 29th data after resampling.")
            feb_29_mask = (hourly_profiles['Zeit'].dt.month == 2) & (hourly_profiles['Zeit'].dt.day == 29)
            feb_29_entries = hourly_profiles[feb_29_mask]
            print(f"Number of February 29th entries to be removed after resampling: {len(feb_29_entries)}")
            hourly_profiles = hourly_profiles[~feb_29_mask]
            print(f"Removed February 29th data after resampling. Remaining rows: {len(hourly_profiles)}")
        else:
            print(f"No leap year detected for {year_of_interest}. No data removed.")

        # Create time series dictionary and profile mapping.
        time_series_dict = {}
        profile_mapping = {}
        print("Creating time series dictionary and profile mapping...")
        for typnummer, df_group in hourly_profiles.groupby('Typnummer'):
            variable_name = f"hourly_profile_{typnummer}"
            if typnummer in constant_profiles:
                constant_value = constant_profiles[typnummer]
                df_group['Wert'] = constant_value
                print(f"Constant profile for Typnummer {typnummer} using constant value {constant_value}")
            time_series_dict[variable_name] = df_group
            profile_mapping[str(typnummer)] = variable_name
            print(f"Created Load Profile '{variable_name}' with {len(df_group)} rows.")
        print("Finished processing CSV data.\n")
        ...
        for _, row in df_mapping.iterrows():
            ...
            if lastprofil in df_time_series.columns:
                ...
                if series_length == 35136:
                    ...
                    print(f"Resampled to hourly. New length: {len(df_sector_series_clean)}\n")
                    ...
                    print(f"Removed February 29th. New length: {len(df_sector_series_clean)}")
                elif series_length == 35184:
                    print(f"Quarter-hourly values detected for profile {lastprofil} (leap year).")
                    df_sector_series_clean = df_sector_series_clean.set_index('time').resample('h').mean().reset_index()
                    df_sector_series_clean.rename(columns={lastprofil: 'Wert'}, inplace=True)
                    print(f"Resampled to hourly. New length: {len(df_sector_series_clean)}")
                    feb_29_mask = (df_sector_series_clean['time'].dt.month == 2) & (df_sector_series_clean['time'].dt.day == 29)
                    ...
                    print(f"Hourly values detected for profile {lastprofil} (normal year). No resampling needed.")
                    df_sector_series_clean.rename(columns={lastprofil: 'Wert'}, inplace=True)
                elif series_length == 8784:
                    print(f"Hourly values detected for profile {lastprofil} (leap year).")
                    df_sector_series_clean.rename(columns={lastprofil: 'Wert'}, inplace=True)
                    feb_29_mask = (df_sector_series_clean['time'].dt.month == 2) & (df_sector_series_clean['time'].dt.day == 29)
                    df_sector_series_clean = df_sector_series_clean[~feb_29_mask]
                    print(f"Removed February 29th. New length: {len(df_sector_series_clean)}")
                else:
                    print(f"Unexpected series length {series_length} for profile {lastprofil}. Skipping.")
                    continue
                if len(df_sector_series_clean) != 8760:
                    print(f"Warning: Length for '{variable_name}' is {len(df_sector_series_clean)}, expected 8760.")
                time_series_dict[variable_name] = df_sector_series_clean
                profile_mapping[str(lastprofil)] = variable_name
                print(f"Created Load Profile '{variable_name}' for sector '{sector}' with {len(df_sector_series_clean)} rows.")
            else:
                print(f"Warning: Lastprofil '{lastprofil}' not found in the time series data. Skipping sector '{sector}'.")
...
def get_sector_weights_for_state(federal_state_name, weight_sectors):
    ...
    if federal_state_name in weight_sectors:
        print(f"Sector weights found for {federal_state_name}.")
        sector_weights_df = weight_sectors[federal_state_name]
        print(sector_weights_df.head())
        return sector_weights_df
    else:
        print(f"No sector weights found for {federal_state_name}.")
        return None
...
def process_federal_state(file_path_load, file_path_mapping, output_directory_time_series, output_directory_aggregated_data, profile_mapping, time_series_dict, year_of_interest, use_weights, weight_file_path, use_excel, excel_profiles_path):
    ...
    def get_federal_state_from_path(file_path):
        ...
        if filename.startswith("Energiebilanz_Verteilung_") and filename.endswith(".xlsx"):
            ...
            print(f"Warning: File name does not match expected pattern: {filename}")
            return None

    federal_state_name = get_federal_state_from_path(file_path_load)
    if federal_state_name in processed_states:
        print(f"Skipping already processed state: {federal_state_name}")
        return
    ...
    if use_excel and excel_profiles_path:
        ...
        print(f"Loaded mapping from {excel_profiles_path}")
    else:
        mapping_df = pd.read_excel(file_path_mapping, sheet_name='Mapping')
        print(f"Loaded mapping from {file_path_mapping}")
    ...
    if sector_weights_df is not None:
        print(f"Sector weights for {federal_state_name}:")
        print(sector_weights_df)
        sector_weights = pd.Series(sector_weights_df.Weight.values, index=sector_weights_df.Sector).to_dict()
    else:
        print(f"No sector weights found for {federal_state_name}. Proceeding without sector-specific weights.")
        sector_weights = None
    ...
    def process_and_generate_hourly_demand_dataframe(file_path_load, time_series_dict, profile_mapping, year_of_interest, use_weights, sector_weights, output_directory_time_series, federal_state_name, use_excel, excel_profiles_path, file_path_mapping):
        ...
        Parameters:
        - file_path_load (str): Path to the input load file.
        - time_series_dict (dict): Dictionary containing time series data.
        - profile_mapping (dict): Mapping of profile IDs to variable names.
        - year_of_interest (int or str): The target year.
        - use_weights (bool): Flag indicating whether to apply weights.
        - sector_weights (dict): Dictionary of sector weights.
        - output_directory_time_series (str): Output directory for time series files.
        - federal_state_name (str): Name of the federal state.
        - use_excel (bool): Flag indicating Excel processing.
        - excel_profiles_path (str): Path to the Excel profiles file.
        - file_path_mapping (str): Path to the mapping file.

        Returns:
        - DataFrame: A DataFrame containing the hourly demand for the federal state.
        """
        print(f"\n--- Starting data processing for file: {file_path_load} ---")
        print(f"Year of Interest: {year_of_interest}, Use Weights: {use_weights}")

        # Load sector data from the file.
        sectors_df = pd.read_excel(file_path_load, sheet_name='Sheet1')
        print(f"Loaded sector data from: {file_path_load}")
        ...
        for index, sector_row in sectors_df.iterrows():
            ...
            print(f"Looking for Lastprofil ID '{lastprofil_id_str}' in profile_mapping...")

            # Validate time series key existence.
            time_series_key = profile_mapping.get(lastprofil_id_str)
            if time_series_key is None:
                print(f"Profile mapping content: {profile_mapping}")
            ...
            if time_series_key not in time_series_dict:
                print(f"Time series dictionary content: {list(time_series_dict.keys())}")
            ...
            if np.isnan(time_series).any():
                print(f"Filling NaN values in time series for sector {sector_id}.")
                time_series = np.nan_to_num(time_series, nan=0.0)

            print(f"Loaded time series for sector '{sector_id}' (Lastprofil ID: {lastprofil_id_str}). First 5 values: {time_series[:5]}")
            ...
            if len(time_series) != expected_length:
                if len(time_series) == 35136:
                    ...
                    print(f"Reshaped time series to {len(time_series)} after aggregation.")
                elif len(time_series) == 8760:
                    print(f"Time series length is already 8760, no adjustment needed.")
                elif len(time_series) == 8761:
                    print(f"Time series has 8761 values, adjusting to 8760 by removing the last value.")
                    time_series = time_series[:8760]
                elif len(time_series) == 8784:
                    print(f"Time series length is 8784, no adjustment needed for leap year.")
                elif len(time_series) == 8759:
                    print(f"Time series has 8759 values, adjusting to 8760 by duplicating the last value.")
            ...
            if np.all(time_series == time_series[0]):
                print(f"Constant time series detected for sector {sector_id}. Normalizing to avoid skewed results.")
                normalized_time_series = time_series / time_series.sum()
            else:
                if time_series.sum() == 0:
                    print(f"Warning: Sum of time series for sector {sector_id} is zero, skipping normalization.")
            ...
            print("-------------")
            print(f"Hourly demand for sector {sector_id}: First 5 values: {hourly_demand[:5]}")
            print(f"Hourly demand for sector {sector_id}: Last 5 values: {demand_df.tail()}")
            print("----------------")
        ...
        print(f"Actual time series length used for column names: {actual_length}")
        ...
        print(f"Created DataFrame with shape: {hourly_demand_df.shape}")
    ...
    if combined_df_8760 is None:
        print(f"Error: Could not generate hourly demand data for {federal_state_name}. Skipping this state.")
        return
    ...
    def aggregate_and_save(combined_df, output_folder, filename, use_weights):
        ...
        print(f"Aggregated DataFrame saved to: {output_path}")
    ...
    def save_created_time_series(combined_df, aggregated_df, output_dir, use_weights, use_excel, federal_state_name, year_of_interest):
        ...
        print(f"Saved combined_df to {combined_df_path}")
        print(f"Saved aggregated_df to {aggregated_df_path}")
        print("\n--------------------------------------------------------------------------------")
